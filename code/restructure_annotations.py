...
if __name__ == "__main__":
    ...
    for idx in range(12,13):
        ...
        for row in sheet.iter_rows(values_only=True):
            ...
            if grounding_act_column is not None:
                grounding_act_column = grounding_act_column.strip()
                if grounding_act_column != 'None' and grounding_act_column != 'Grounding Act':
                    current_ids = [int(float(i.strip())) if '*' not in i else i.strip() for i in str(row[5]).split(',')]
                    ...
                    for i in range(len(current_ids)):
                        ...
                        if current_acts[i] == 'Explicit-Ack':
                            ...
                            degree_of_grounding.append('medium')
                        # except:
                        #     print(current_ids, current_acts)
